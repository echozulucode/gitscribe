use std::path::Path;
use context_core::{generate_context, call_ollama, read_file_content, list_git_refs};
...
#[tauri::command]
fn generate_ai_cmd(repo_path: String, start: String, end: String, notes: String, model: String, system_prompt: Option<String>) -> Result<String, String> {
    let context = generate_context(&start, &end, Some(notes), Some(Path::new(&repo_path)))
        .map_err(|e| e.to_string())?;
    
    let url = "http://localhost:11434/api/generate";
    
    call_ollama(&model, url, &context, system_prompt.as_ref())
        .map_err(|e| e.to_string())
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![generate_preview_cmd, generate_ai_cmd, load_file_cmd, get_repo_refs_cmd])
